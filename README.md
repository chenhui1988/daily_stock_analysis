...
> 🤖 基于 AI 大模型的 A股/港股/美股/Bursa Malaysia 自选股智能分析系统，每日自动分析并推送「决策仪表盘」到企业微信/飞书/Telegram/Discord/邮箱
...
| 市场 | 全球市场 | 支持 A股、港股、美股、Bursa Malaysia（`.KL`）及美股指数（SPX、DJI、IXIC 等） |
...
>
> 注：Bursa Malaysia 当前为第一阶段支持，使用 Yahoo Finance 代码格式 `XXXX.KL`（如 `5183.KL`、`1155.KL`）；已覆盖单股分析、行情、导入、Bot/API/Web 输入与持仓市场枚举。`MARKET_REVIEW_REGION` 仍仅支持 `cn / us / both`。
...
| Secret 名称 | 说明 | 必填 |
|------------|------|:----:|
| `STOCK_LIST` | 自选股代码，如 `600519,hk00700,AAPL,5183.KL` | ✅ |
