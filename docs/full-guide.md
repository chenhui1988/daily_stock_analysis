...
| Secret 名称 | 说明 | 必填 |
|------------|------|:----:|
| `STOCK_LIST` | 自选股代码，如 `600519,300750,5183.KL` | ✅ |
...
| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| `STOCK_LIST` | 自选股代码（逗号分隔，支持如 `600519,hk00700,AAPL,5183.KL`） | - |
...
### YFinance
- 免费，无需配置
- 支持美股/港股/Bursa Malaysia（`.KL`）数据
- 美股历史数据与实时行情均统一使用 YFinance，以避免 akshare 美股复权异常导致的技术指标错误
- Bursa Malaysia 当前为第一阶段支持：请显式使用 `XXXX.KL`，如 `5183.KL`、`1155.KL`
...
使用 Yahoo Finance 的 `.KL` 后缀指定 Bursa Malaysia 股票代码：

```bash
STOCK_LIST=600519,hk00700,AAPL,5183.KL
```

说明：

- 当前支持范围为单股分析、实时行情、历史行情、图片/导入解析、Bot/API/Web 输入链路，以及持仓账户市场 `my`
- 请显式使用 `XXXX.KL` 格式，不要只填裸 4 位或 5 位马股代码
- `MARKET_REVIEW_REGION` 仍只支持 `cn`、`us`、`both`，本轮不新增 `my`
...
| `/api/v1/stocks/{code}/quote` | GET | 获取实时行情（支持 `5183.KL`） |
| `/api/v1/stocks/{code}/history` | GET | 获取历史行情（支持 `5183.KL`） |
| `/api/health` | GET | 健康检查 |
...
# 获取 Bursa Malaysia 实时行情
curl http://127.0.0.1:8000/api/v1/stocks/5183.KL/quote
...
| Bursa Malaysia | `XXXX.KL` | `5183.KL`、`1155.KL` |
| 美股指数 | SPX/DJI/IXIC 等 | `SPX`、`DJI`、`NASDAQ`、`VIX` |

### 注意事项

- 浏览器访问：`http://127.0.0.1:8000`（或您配置的端口）
- Bursa Malaysia 请使用显式 `.KL` 代码格式；当前不支持把裸数字自动推断为马股
...
### Q: 如何添加自选股？
A: 修改 `STOCK_LIST` 环境变量，多个代码用逗号分隔，例如 `600519,hk00700,AAPL,5183.KL`。
